...
class ItemsView extends StatelessWidget {
  ...
  ItemsView({@required this.stores, @required this.items, @required this.isStore, this.isScrollable = false,
    ...
    this.isCampaign = false, this.inStorePage = false, this.type, this.onVegFilterTap, this.isFeatured = false});
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      ...
      !_isNull ? _length > 0 ? GridView.builder(
        ...
        padding: padding,
        itemBuilder: (context, index) {
          return ItemShimmer(isEnabled: _isNull, isStore: isStore, hasDivider: index != shimmerLength-1);
        },
      ),
