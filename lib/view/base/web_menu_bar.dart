...
class WebMenuBar extends StatelessWidget implements PreferredSizeWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if(Get.find<SplashController>().configModel.toggleStoreRegistration) {
      _entryList.add(PopupMenuItem(child: Text(Get.find<SplashController>().configModel.moduleConfig.module.showRestaurantText
          ? 'join_as_a_restaurant'.tr : 'join_as_a_store'.tr), onTap: () async {
        if(await canLaunch('${AppConstants.BASE_URL}/store/apply')) {
          launch('${AppConstants.BASE_URL}/store/apply');
        }
      }));
    }
